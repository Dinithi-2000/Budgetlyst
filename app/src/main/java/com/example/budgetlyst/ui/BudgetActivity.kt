...
import com.example.budgetlyst.databinding.ActivityBudgetBinding
...
private fun setupBudgetDisplay() {
    ...
    val currencySymbol = preferencesManager.getCurrencySymbol() // Use symbol
    ...
    if (budget.month == month && budget.year == year && budget.amount > 0) {
        ...
        binding.tvBudgetAmount.text = String.format("%s%.2f", currencySymbol, budget.amount)
        binding.tvExpensesAmount.text = String.format("%s%.2f", currencySymbol, totalExpenses)
        binding.tvRemainingAmount.text = String.format("%s%.2f", currencySymbol, remaining)
