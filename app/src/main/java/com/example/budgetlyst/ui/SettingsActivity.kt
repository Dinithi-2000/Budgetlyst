...
import com.example.budgetlyst.databinding.ActivitySettingsBinding
...
class SettingsActivity : AppCompatActivity() {
    ...
    private fun setupCurrencySpinner() {
        val currencies = arrayOf("USD", "LKR", "EUR", "GBP", "JPY", "CAD", "AUD", "INR", "CNY")
    ...
    private fun setupBackupButtons() {
        binding.btnBackupData.setOnClickListener {
            ...
            // Create backup in internal storage
            val timestamp = SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(Date())
            val backupFileName = "transactions_backup_$timestamp.json"
            val backupFile = File(filesDir, backupFileName)

            try {
                if (transactionRepository.backupToFile(this, backupFile)) {
                    Toast.makeText(
                        this,
                        "Backup saved to internal storage: $backupFileName",
                        Toast.LENGTH_LONG
                    ).show()
                } else {
    ...
    private fun restoreFromLastBackup() {
        // Find the latest backup file in internal storage
        val backupFiles = filesDir.listFiles { _, name ->
            name.startsWith("transactions_backup_") && name.endsWith(".json")
        }
        ...
        val latestBackupFile = backupFiles.sortedByDescending { it.name }.first()

        AlertDialog.Builder(this)
            .setTitle("Warning")
            .setMessage("This will replace all existing transactions with data from ${latestBackupFile.name}. Continue?")
            .setPositiveButton("Yes") { _, _ ->
                if (transactionRepository.restoreFromFile(this, latestBackupFile)) {
